...
def run_scraper(cookies, api_key, urls):
    ...
    results = scrape_cookies.process_profiles(cookies, urls, api_key) #from scrape.py
...
def status():
    ...
    if is_scraping_complete:
        return {"status": "completed",
                "current": scrape_cookies.progress['current'], #pull what profile we're on from scrape.py "progress" variable
                "total": scrape_cookies.progress['total']
                }, 200
    else:
        return {"status": "in_progress",
                "current": scrape_cookies.progress['current'],
                "total": scrape_cookies.progress['total']
                }, 202
